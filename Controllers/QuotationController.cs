...
namespace GeekHackers_WebAPI.Controllers
{
    ...
    public class QuotationController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/Quotation/getQuotationByID")]
        public HttpResponseMessage getQuotationByID(int id)
        {
            using (SMME_Hackathon2022Entities db = new SMME_Hackathon2022Entities())
            {
                try
                {
                    ...
                    var getRecord = db.Quotations.Where(m => m.QuotationID == id).FirstOrDefault();
                    if (getRecord != null)
                    {
        ...
        //Create Quotations
        [HttpPost]
        [Route("api/Quotation/SaveFile")]
        public HttpResponseMessage SaveFile([FromBody] Quotation QuotationData)
        {
            ...
            db.Quotations.Add(QuotationData);
            db.SaveChanges();
            return Request.CreateResponse(HttpStatusCode.OK);
        }
        [HttpPost]
        [Route("api/Quotation/AddQuotation")]
        public HttpResponseMessage AddQuotation()
        {
            ...
            using (SMME_Hackathon2022Entities db = new SMME_Hackathon2022Entities())
            {
                Quotation QuotattionItem = new Quotation()
                {
                    ItemName = httpRequest["ItemName"],
                    ItemDescription = httpRequest["ItemDescription"],
                    CompanyName = httpRequest["CompanyName"],
                    Date_Created = DateTime.Now,
                    Quantity = Convert.ToInt32(httpRequest["Quantity"]),
                    Unit_Price = Convert.ToDecimal(httpRequest["Unit_Price"]),
                    Total_Price = Convert.ToDecimal(httpRequest["Total_Price"]),
                    ImageName = Imagename,
                    ImagePath = filepath,


                };
                db.Quotations.Add(QuotattionItem);
                db.SaveChanges();
        ...
        public HttpResponseMessage editQuotation()
        {
            using (SMME_Hackathon2022Entities db = new SMME_Hackathon2022Entities())
            {
                var httpRequest = HttpContext.Current.Request;
                int id = Convert.ToInt32(httpRequest["id"]);
                var checkRecord = db.Quotations.Where(xx => xx.QuotationID == id).FirstOrDefault(); ;
                if (checkRecord != null)
                {
                    string imageName = null;
                    //Upload image
                    ...
                    checkRecord.ItemName = httpRequest["ItemName"];
                    checkRecord.ItemDescription = httpRequest["ItemDescription"];
        ...
        public HttpResponseMessage deleteQuotation(int id)
        {
            using (SMME_Hackathon2022Entities db = new SMME_Hackathon2022Entities())
            {
                try
                {
                    ...
                    if (getRecord != null)
                    {
                        db.Quotations.Remove(db.Quotations.Where(m => m.QuotationID == id).FirstOrDefault());
                        db.SaveChanges();
                        return Request.CreateResponse(HttpStatusCode.OK);
                    }
                    else
